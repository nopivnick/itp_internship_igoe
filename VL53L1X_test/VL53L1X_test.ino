/*
  VL53L1X Time-of-flight .csv data dump for sensor tweaking
  Uses Sparkfun's VL53L1X library: http://librarymanager/All#SparkFun_VL53L1X
  
  Prints the following data separated by comma to serial:
  - distanceMode
  - distanceMode (human readable)
  - distanceOffset
  - timingBudget
  - timingBudgetFactor
  ...
  - distance
  - rangeStatus
  - rangeStatus (human readable)

  To begin capturing the data from terminal:
  $ cat /dev/cu.usbmodem14101 > log.csv
  To stop capturing the data from terminal press Ctrl+C
  ...
  https://github.com/tigoe/SensorExamples/tree/master/TimeOfFlightSensors/VL53L1X_errors
*/
...
float timingBudgetFactor = 1.5; // programmatically ensure intermeasurementPeriod is >= timingBudget
...
// in practice, distance readings beyond 2m in a lit environment probably require 200 or 500ms.
...
void setup() {
  ...
  sensor.setIntermeasurementPeriod(timingBudget * timingBudgetFactor);
  //  sensor.setOffset(29); // uncomment if setting offset manually
}

void loop() {
  int distanceOffset = sensor.getOffset();
  int intermeasurementPeriod = sensor.getIntermeasurementPeriod();
  // delay for intermeasurement period:
  long timeSinceReading = millis() - lastReadingTime;
  if (timeSinceReading > intermeasurementPeriod) {
    ...
    Serial.print(distanceMode); // short = 1, long = 2
    Serial.print(",");

    if (distanceMode == 1) {
      Serial.print("Short Distance Mode");
    }
    else if (distanceMode == 2) {
      Serial.print("Long Distance Mode");
    }
    else {
    ...
    Serial.print(distanceOffset); // in millimeters
    Serial.print(",");

    Serial.print(timingBudget); // in milliseconds
    Serial.print(",");

    Serial.print(timingBudgetFactor);
    Serial.print(",");

    Serial.print(intermeasurementPeriod); // in milliseconds
    Serial.print(",");

    Serial.print(timeSinceReading); // in milliseconds
    Serial.print(",");
    ...
    // make rangeStatus human readable
    switch (rangeStatus)  {
      case 0:
